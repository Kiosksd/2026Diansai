...
- 本车物理左轮是 `CH2/TIMG8`，物理右轮是 `CH1/TIMG9`，不得再按通道编号猜测左右。
...
- 全黑图案不再表示终点，也不会触发停车；当前版本仅通过 KEY1、丢线保护或直角转向超时停止。
...
约束：不要按“编码器 1/2”或“通道 1/2”的名称猜测左右轮。本车 `CH1=物理右轮`、`CH2=物理左轮`，且电机与编码器为交叉连接。速度控制器统一使用“车辆前进为正”，所以必须按上表修正左轮反馈符号。

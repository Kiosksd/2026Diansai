...
> 新电机的手动编码器、单通道正反转和电机/编码器配对已经完成。以下 2026-07-25 表格是当前唯一有效映射。工程仍启用 `NEW_MOTOR_SAFE_WIRELESS_TEST=1`，当前子阶段为 `SAFE_TEST_STAGE_SINGLE_WHEEL_PI`：`L`/`R` 分别启动左/右单轮前进 PI，`B` 启动双轮独立 PI，目标 `8 count/10ms`、运行 3000 ms、PWM 上限 2000，`0` 可提前停止；空闲时发送 `G` 可在两轮 PWM=0 的前提下读取一次 GS08RA 的 BIN、偏差、原始值和归一化值。单轮架空首测平均为左 `8.62`、右 `8.64 count/10ms`，无反向和饱和；据末段稳态 PWM 将测试前馈修正为 `250 + 105 × target`。双轮架空测试为左 `8.06`、右 `8.11`，落地直行测试为左 `7.50`、右 `7.52`，稳定阶段均保持约 `8 count/10ms`，实车确认直行。

...
| `SERIAL_P8` | 主板“串口接口” P8 | TX/RX | TX `B15`=`UART7_TX_B15`；RX `B16`=`UART7_RX_B16` | P8 同时提供 `VCC5V/GND`；当前本车用于八路红外巡线传感器。 | `CONFIRMED` |
...
#### 本车八路红外巡线传感器 UART 约定（实测）

- 传感器使用 P8 `UART_7`：MCU TX=`B15`=`UART7_TX_B15` -> 传感器 RX；传感器 TX -> MCU RX=`B16`=`UART7_RX_B16`。
- 传感器供电为 P8 `VCC5V/GND`；UART 默认 `115200, 8N1`。P6 的 `SCL/SDA` 在 UART 方案中不连接。
- 上电后主控发送 ASCII 命令 `$0,0,1#` 开启数字量输出；无此命令时模块默认不发送数据。
- 数字帧固定为 `$D,x1:0,x2:0,x3:0,x4:0,x5:0,x6:0,x7:0,x8:0#`；以 `$` 开始，以 `#` 结束。
- `X1..X8` 映射为原始字节 `bit7..bit0`；本车实测 `X1` 在物理左侧，`X8` 在物理右侧，不需要软件反序。
- 黑线为 `0`，白色为 `1`；实测物理左侧 X1 单独压黑时 `RAW=0x7F / BIN=01111111`。
- 工程驱动文件为 `project/user/inc/line_sensor_ir8_uart.h` 和 `project/user/src/line_sensor_ir8_uart.c`；应用控制层只读取解析后的八路数组，不应重复解析 ASCII 帧。
- 串口接收使用 UART7 RX 中断；无线调试仍使用 P12 `UART_1`，两者可同时工作。P8 被红外传感器占用后，不能再同时接 UART7 陀螺仪。
- 运行时连续 `100 ms` 没有新的有效帧必须强制停车；无数据时不得继续使用旧的 `BIN`。停车后每 `1000 ms` 可重发一次数字量请求命令。
- 有效 UART 帧为全白 `BIN=11111111` 时不触发丢线停车：以 `5 count/10ms` 的低速按最后一次偏差方向持续寻线；没有已知方向时每 `100 ms` 小幅左右交替搜索，重新检测到黑线后立刻恢复正常循迹。UART `100 ms` 无有效帧仍属于通信故障并强制停车，不受该规则影响。
- 2026-07-29 稳定性实测：全白、全黑、单通道及长时间连续收数据均通过；一路可见指示灯不亮，但对应数字通道能正常变为 `0`，不影响巡线输入。
- 2026-07-29 IR8 首轮提速配置 `18/15/12 count/10ms` 实测圈时 `37.170 s`，硬件 PIT 计时与实测基本一致；程序按 20 秒目标给出 `scale_x1000=1858`。下一档采用直线/缓弯/弯道 `33/28/22`、差速上限 `15`、轮速目标上限 `40`，速度 PI、循迹 PD、PWM 上限 `4500` 和终点线判断保持不变，等待实车复测。
...
> 新电机的手动编码器、单通道正反转和电机/编码器配对已经完成。以下 2026-07-25 表格是当前唯一有效映射。工程仍启用 `NEW_MOTOR_SAFE_WIRELESS_TEST=1`。开环映射、单/双轮速度 PI 和低速循迹分别保留为 `SAFE_TEST_STAGE_OPEN_LOOP`、`SAFE_TEST_STAGE_SINGLE_WHEEL_PI`、`SAFE_TEST_STAGE_LOW_SPEED_TRACK` 三个可切换阶段。当前启用循迹阶段：`S`/`1` 启动并持续运行，`P`/`0` 主动停止，`G` 在停车时读取灰度快照，`?` 查询状态；根据硬件PIT准确圈时继续标定，直道、轻弯、明显弯道目标分别为 `28/24/20 count/10ms`；转向差速缩放为3、上限为 `15`，目标速度上限为 `35`，PWM 上限为 4500。不设固定运行时长，连续丢线 150 ms 强制停止。单轮架空首测平均为左 `8.62`、右 `8.64 count/10ms`，无反向和饱和；速度 PI 前馈为 `250 + 105 × target`。双轮架空测试为左 `8.06`、右 `8.11`，落地直行测试为左 `7.50`、右 `7.52`，稳定阶段均保持约 `8 count/10ms`，实车确认直行。
...
2026-07-29 按实车提速要求，将已经验证的低速版全部轮速目标整体乘以2：分级基础速度由 `8/7/6` 改为 `16/14/12`，差速修正和目标上限同步乘以2，以保持近似转弯半径；PWM 上限由 2000 提高到 3000。该高速配置需要重新完成直线、缓弯和丢线保护实车复测。

赛道为两段 1.5 m 直线和两端半径 0.5 m 的半圆，总长约 `3+π=6.142 m`；20秒目标对应平均线速度约 `0.307 m/s`。由于新电机/车轮尚无“编码器计数每米”标定，采用实测圈时比例标定。高速版增加目标速度斜坡（每10 ms加1、减2）抑制起步和直弯切换冲击。A点停车线沿行驶方向宽约5 cm；为避免提速后漏过短停车线，终点条件改为启动12秒后任一10 ms控制周期检测到黑色通道数不少于4即锁存停车，12秒时间门限用于排除起点处的停车线。首圈使用 `16/14/12` 时实测 `28.85 s`，停车线在 `black=4, peak=4` 时正确触发，随后按比例调整为 `23/20/17`。第二圈实测 `20.98 s`，全程无持续丢线、PWM未达到4000上限，停车线仍在 `black=4, peak=4` 时正确触发；程序给出 `scale_x1000=1049`，现已进行最后小幅修正为 `24/21/18`，差速上限13、目标上限31，PI/PD增益保持不变。

> 2026-07-29 圈时修正：旧版在包含灰度扫描、无线发送和 `system_delay_ms(10)` 的前台循环中，每轮固定增加10 ms，因此实际 `22.9 s` 被少记为 `19.79 s`。现改为在10 ms硬件PIT速度环中断中累计圈时，前台计算和串口耗时不再影响 `TRACK t`、12秒终点门限及 `LAP time`。旧圈时及其自动速度建议仅可作为历史记录，后续调速必须以修正后的 `LAP time` 为准。

> 修正计时后的首圈为 `22.51 s`，终点在 `black=4, peak=4` 时正确停车。按 `scale_x1000=1125` 调整为 `27/24/20`，差速上限15；为避免新速度和差速被旧限幅截断，目标上限同步改为35，PWM上限按相同比例由4000改为4500。PI/PD增益、目标斜坡、12秒终点门限和150 ms丢线保护保持不变。

> `27/24/20` 实测圈时为 `20.38 s`，全程最大误差3、最大转向输出9、无丢线和PWM饱和，PD参数保持不变。按程序建议只将直道目标小幅提高到28，形成当前 `28/24/20` 配置。
